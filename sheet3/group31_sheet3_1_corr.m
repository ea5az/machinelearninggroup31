...
for i = 1:3
    select = (Y > (i-1.)/3.) & (Y <= i/3.); %TUTOR:ok..but that's defenetily not the easiest way...!
...
while((mu_delta > eps && sigma_delta > eps) || first_run) %while change in values 'big'
    ...
    p(: , 2) = p(: , 2) .* (1./normsum);
    p(: , 3) = p(: , 3) .* (1./normsum);
	%TUTOR: The Likelyhoods are normalized like that!
...
%TUTOR: Your code doesn't run. However, the code as such looks very good. Please hand in running code only. 7/10 points
